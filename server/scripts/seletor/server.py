# seletor.py (VERSÃO CORRIGIDA - Sem erro de nó)
...
def show_login_gateway():
    """Exibe uma página de portal limpa com botão centralizado."""
    st.set_page_config(layout="centered", initial_sidebar_state="collapsed")
    
    # CSS simplificado para evitar conflitos com o DOM do Streamlit
    st.markdown("""
    <style>
    /* Esconde elementos do Streamlit de forma mais segura */
    header, .st-emotion-cache-16txtl3, div[data-testid="stToolbar"], 
    ...
    # HTML simplificado para evitar manipulação excessiva do DOM
    st.markdown(f"""
    ...
    # Para garantir que não haja conteúdo adicional do Streamlit
    st.empty()
...
def main():
    ...
    # Verifica o token na query string
    query_params = st.query_params
    token = query_params.get("token", None)
    
    if token and verify_token(token):
        # Se o token for válido, mostra o hub de plataformas
        show_platforms_hub()
    else:
        # Se não houver token ou for inválido, mostra o portal de acesso
        show_login_gateway()
